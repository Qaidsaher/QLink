...
<div class="post-action-wrapper">
    ...
    <div class="flex items-center justify-between max-w-sm mt-4 text-gray-500">
        ...
        <button wire:click="toggleComments({{ $post->id }})" class="flex items-center space-x-2 transition group">
        ...
        <button class="flex items-center space-x-2 transition group">
            <div class="p-2 rounded-full group-hover:bg-green-100 dark:group-hover:bg-green-900/20">
                <svg class="w-5 h-5 group-hover:text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 9a8 8 0 0114.24-4.76L20 5"></path>
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 15a8 8 0 01-14.24 4.76L4 19"></path>
                </svg>
            </div>
        ...
        <button wire:click="toggleLike({{ $post->id }})" class="flex items-center space-x-2 transition group">
            <div class="p-2 rounded-full group-hover:bg-red-100 dark:group-hover:bg-red-900/20">
                <svg class="w-5 h-5 {{ $post->isLikedBy(auth()->user()) ? 'text-red-500' : 'group-hover:text-red-500' }}" fill="{{ $post->isLikedBy(auth()->user()) ? 'currentColor' : 'none' }}" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                </svg>
            </div>
            <span class="text-sm {{ $post->isLikedBy(auth()->user()) ? 'text-red-500' : 'group-hover:text-red-500' }}">{{ $post->likes->count() }}</span>
        </button>

        {{-- Share Button --}}
        <button class="flex items-center p-2 transition rounded-full group hover:bg-blue-100 dark:hover:bg-blue-900/20">
            <svg class="w-5 h-5 group-hover:text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 12v8a2 2 0 002 2h12a2 2 0 002-2v-8m-4-6l-4-4m0 0L8 6m4-4v12" />
            </svg>
        </button>
    </div>
    ...
    <div x-data="{ openComments: @entangle('openCommentsSection.' . $post->id) }" x-show="openComments" x-transition:enter="transition ease-out duration-300"
        ...
        @auth
            <div class="flex w-full space-x-4">
                <img src="{{ Auth::user()->avatarUrl() }}" alt="{{ Auth::user()->name }}" class="object-cover w-10 h-10 rounded-full">
                <div class="flex-1">
                    {{-- (!-- FIX: Wire model to the specific post's comment text in the array --) --}}
                    <textarea wire:model.defer="newCommentText.{{ $post->id }}" placeholder="Post your reply" rows="2" class="w-full p-2 text-base bg-transparent border-b border-gray-300 rounded-lg dark:border-slate-600 focus:outline-none focus:border-blue-500 dark:text-white"></textarea>
                    <div class="flex items-center justify-end mt-2">
                        {{-- (!-- FIX: Point the error message to the correct array key --) --}}
                        @error('newCommentText.' . $post->id) <span class="mr-auto text-xs text-red-500">{{ $message }}</span> @enderror
                        {{-- (!-- FIX: Pass the post ID to the addComment method --) --}}
                        <button wire:click="addComment({{ $post->id }})" wire:loading.attr="disabled" wire:target="addComment" class="px-4 py-1.5 text-sm font-bold text-white bg-blue-500 rounded-full hover:bg-blue-600 disabled:opacity-50 min-w-[70px] text-center">
                    ...
                    <div class="flex-1">
                        <div class="relative p-3 bg-gray-100 rounded-lg dark:bg-slate-800">
                            <div class="flex items-center space-x-2 text-sm">
                                ...
                                <span class="text-gray-500 dark:text-gray-400">@<span>{{ $comment->user->username }}</span></span>
                                @if(Auth::id() === $comment->user_id)
                                    {{-- (!-- FIX: Call the new confirmation method --) --}}
                                    <button wire:click="requestDeleteConfirmation({{ $comment->id }})" class="ml-auto text-gray-400 hover:text-red-500 focus:outline-none" title="Delete Comment">
...
<div x-data="{ show: @entangle('confirmingCommentDeletion') }" x-show="show" x-trap.noscroll="show"
    ...
    <div @click.away="show = false" class="relative w-full max-w-md p-6 mx-auto bg-white shadow-lg rounded-xl dark:bg-slate-800">
        <h2 class="mb-4 text-lg font-semibold text-gray-800 dark:text-gray-100">Delete Comment?</h2>
        <p class="mb-6 text-sm text-gray-600 dark:text-gray-300">
            Are you sure? This action is permanent.
        </p>
        <div class="flex justify-end space-x-3">
            <button @click="show = false" class="px-5 py-2 text-gray-700 transition bg-gray-100 rounded-md dark:text-gray-300 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600">
                Cancel
            </button>
            {{-- (!-- FIX: Call the new parameter-less deleteComment method --) --}}
            <button wire:click="deleteComment" wire:loading.attr="disabled" class="px-5 py-2 text-white transition bg-red-600 rounded-md hover:bg-red-700">
